...
func (s *XSqlOrder) Begin() {
	var err error
	s.xs.tx, err = s.xs.db.Begin()
	if err != nil {
		log.Println(err)
		return
	}
}
func (s *XSqlOrder) Stop() {
	err := s.xs.tx.Commit()
	if err != nil {
		log.Println(err)
		return
...
func (s *XSqlOrder) RollBack() {
	err := s.xs.tx.Rollback()
	if err != nil {
		log.Println(err)
		return
	}
...
func (s *XSqlOrder) Select(name string, keys ...string) { //第一个参数为列表名，第二个为参数类型，int，string，float
	...
	log.Println(s.reqBuffer.String())
		...
		case string:
			{
				...
				valueString.WriteString("\"")
				valueString.WriteString(values[key].(string))
				valueString.WriteString("\"")
				if index != len(values) {
					valueString.WriteString(",")
				}
...
func (s *XSqlOrder) MulitInsert(list []map[string]interface{}, name string) {
	...
	log.Println("reqString: ", s.reqBuffer.String())
	log.Println("valueString: ", valueString)
...
func (s *XSqlOrder) InsertPerpare(values map[string]interface{}, name string) {
	...
	for key, _ := range values {
		...
		case string:
			{
				s.insertValues[index] = values[key].(string)
			}
			break
		case []byte:
			{
				s.insertValues[index] = string(values[key].([]byte))
			}
			break
		}
		index++
	...
	s.reqBuffer.WriteString(")")
	//s.reqString = Substr(reqString, 0, len(reqString)-1)
	log.Println(s.reqBuffer.String(), s.insertValues)
...
func (s *XSqlOrder) Qurey(suffixes string) {
	...
	// 提前写好要准备的语句，再通过传参 调用 ArgsForPerpare。
	//s.colType = make(map[string]string)
	s.ClearBuffer()
...
func (s *XSqlOrder) ArgsForPerpare(args []interface{}) {

	s.whereValues = make([]interface{}, len(args))
	s.whereValues = args
}
		...
		case string:
			{
				s.reqBuffer.WriteString(key)
				s.reqBuffer.WriteString("=?")
				s.whereValues[index] = values[key].(string)
			}
			break
		case []byte:
			{
				s.reqBuffer.WriteString(key)
				s.reqBuffer.WriteString("=?")
				s.whereValues[index] = string(values[key].([]byte))
			}
			break
		}
		index++
...
func (s *XSqlOrder) ExecuteNoResult() {
	...
	s.xs.mLock.RLock()
	go timer(s)
	_, err := s.xs.db.Exec(s.reqBuffer.String())
	if err != nil {
		log.Println("ExecuteNoResult执行sql语句: " + s.reqBuffer.String())
		log.Println(err)
	}
	s.xs.mLock.RUnlock()
	s.ch = 1
...
func (s *XSqlOrder) ExecuteForLastInsertId() int64 {
	...
	if err != nil {
		log.Println("ExecuteForLastInsertId执行sql语句: " + s.reqBuffer.String())
		log.Println("err:", err)
	}
...
func (s *XSqlOrder) Execute2() []map[string]interface{} { //SQL
	...
	s.ch = 1
	if err != nil {
		log.Println("Execute执行sql语句: " + s.reqBuffer.String())
		log.Println("error: ", err)
		s.xs.mLock.RLock()
		s.xs.db.Close()
		db := createDB(s.xs.name, s.xs.password, s.xs.ip, s.xs.port, s.xs.sqlName)
	...
	if err2 != nil {
		log.PrintError(err2) // proper error handling instead of panic in your app
		return nil
	}
...
func (s *XSqlOrder) ExecuteNoResultPerpare() {
	...
	var err error
	if s.xs.tx != nil {
		rows, err = s.xs.tx.Query(s.reqBuffer.String(), args...)
	} else {
	...
	if err != nil {
		log.Println("ExecuteNoResult执行sql语句: " + s.reqBuffer.String())
		log.Println("ExecuteNoResult执行sql参数: ", args)
		log.Println(err)
		s.insertValues = nil
		s.whereValues = nil
		s.xs.mLock.RUnlock()
...
func (s *XSqlOrder) ExecuteForLastInsertIdPerpare() int64 {
	...
	var err error
	if s.xs.tx != nil {
		ret, err = s.xs.tx.Exec(s.reqBuffer.String(), args...)
	} else {
		ret, err = s.xs.db.Exec(s.reqBuffer.String(), args...)
	}
	if err != nil {
		log.Println("ExecuteForLastInsertId执行sql语句: " + s.reqBuffer.String())
		log.Println("ExecuteForLastInsertId执行sql参数: ", args)
		log.Println("err:", err)
		s.insertValues = nil
		s.whereValues = nil
		s.xs.mLock.RUnlock()
...
func (s *XSqlOrder) ExecutePerpare() []map[string]interface{} { //SQL
	...
	if err != nil {
		log.Println("Execute执行sql语句: " + s.reqBuffer.String())
		log.Println("Execute执行sql参数: ", args)
		log.Println("error: ", err)
		s.xs.mLock.RLock()
		s.xs.db.Close()
		db := createDB(s.xs.name, s.xs.password, s.xs.ip, s.xs.port, s.xs.sqlName)
